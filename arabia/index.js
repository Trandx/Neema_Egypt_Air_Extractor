...
(function() {
    ...
    const _regex_ancillary_details = [
        ...
        /\w{3}\/\w{3}\s+\w\d{4}\s+(?<go_meal>.+)\s+(?<go_bagages>\d+(\s)?Kg).+\s.+\s+(?<back_meal>.+)?\s+(?<back_bagages>\d+(\s)?Kg)/m,
        ///\w{3}\/\w{3}\s+\w\d{4}\s+(?<go_meal>.+)\s(?<go_bagages>\d+(\s)?Kg)/mg,
        /\w{3}\/\w{3}\s+\w\d{4}\s+(?<seat>\d+\w{1})?\s+(?<go_meal>.+)\s(?<go_bagages>\d+(\s)?Kg)/mg
    ] 
