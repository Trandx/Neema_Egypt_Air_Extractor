import fileDatas from "./datas.mjs";
                       ...
                       (() =>{
                            ...
                            const _regex_ticket_number = /Ticket number\s*:\s*(\d{3})\s(\d+)?\s-?\s(\d*)/i
    
                            const _regex_traveler_name =  /Traveler\s*([\w\s]+)\s/i // /Traveler\s*\w{1,3}\s([\w\s]+)\s/i
    
                            const _regex_pnr = /Booking ref:\s*(\w+)/i
    
                            const _regex_issuing_date = /Issue date:\s*(\d{2})\s(\w+)\s(\d{2})/i

                            const _regex_airline_iata = /Airline booking ref:\s*(\w+)/i

                            const _regex_airline = /Issuing Airline:\s*([\w\s]+)\n/i

                            const _regex_iata = /IATA\s*([\w]+)\n/i
    
                            const _regex_total_price = /Total Amount.+\:\s\w+\s(\d+)/i

                            const _regex_form_of_payement = /Form of payment\s+:\s*(\w+)/i

                            const _regex_id_current = /Tax\s+:\s*(\w{3})/i
                            ...
                            const _regex_franchise = /Baggage:\s*(\w+)/igm
                            ...
                            const _regex_city_and_flight_number = /(?<from>(?:\w{3,}\s){1,4})\s*(?<to>(?:\w{3,}\s)+)\s*(?<flight>[A-Z]{2}\d{2,4})/ig 
                            // /((?:\w{3,}\s){2,}INTL)(?:\s*)((?:\w{3,}\s){2,})(?:\s*)((?:\w{2}\d+))/mg
                            ...
                            console.log(ticker_numb_details)
                            booking_data.ticket_number = ticker_numb_details[1]
                            booking_data.conjunction_number = ticker_numb_details[2]
                            ...
                            //get total_price
                            const total_price = _matcher(fileDatas, _regex_total_price)
                            booking_data.total_price = !isNaN(total_price)?total_price:0
    
    
                            //get total_air_taxes
                            const tb_taxes = _extract_taxes(fileDatas, _regex_taxes)
    
                            booking_data.total_air_taxes = isNaN(total_price)?[]:_compute_taxes(tb_taxes)
